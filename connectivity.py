...
from mn_wifi.node import Station, OVSKernelAP, RemoteController*
...
def topology(args):
 ...
 info("*** Creating server room equipment \n")
    ...
    info("*** Creating switch for RD servers on ground floor \n")
    ...
    info("*** Creating switch for Management Team Office and Demo Room \n")
    ...
    swbldg2 = net.addSwitch('swbldg2',cls=OVSKernelSwitch)
    ...
    sw1.cmd('ovs-vsctl set port sw1-eth24 trunks=8,10,11,12,13,14,15,55')
    sw2.cmd('ovs-vsctl set port sw2-eth24 trunks=8,10,11,12,13,14,15,55')
    sw2.cmd('ovs-vsctl set port sw2-eth23 trunks=8,10,11,12,13,14,15,55')
    sw3.cmd('ovs-vsctl set port sw3-eth23 trunks=8,10,11,12,13,14,15,55')
    sw3.cmd('ovs-vsctl set port sw3-eth22 trunks=8,10,11,12,13,14,15,55')
    sw3.cmd('ovs-vsctl set port sw3-eth21 trunks=8,10,11,12,13,14,15,55')
    sw3.cmd('ovs-vsctl set port sw3-eth24 trunks=8,10,11,12,13,14,15,55')
    sw4.cmd('ovs-vsctl set port sw4-eth24 trunks=8,10,11,12,13,14,15,55')
    sw4.cmd('ovs-vsctl set port sw4-eth23 trunks=8,10,11,12,13,14,15,55')
    sw5.cmd('ovs-vsctl set port sw5-eth24 trunks=8,10,11,12,13,14,15,55')
    sw5.cmd('ovs-vsctl set port sw5-eth23 trunks=8,10,11,12,13,14,15,55')
    sw6.cmd('ovs-vsctl set port sw6-eth24 trunks=8,10,11,12,13,14,15,55')
    sw6.cmd('ovs-vsctl set port sw6-eth23 trunks=8,10,11,12,13,14,15,55')
    sw7.cmd('ovs-vsctl set port sw7-eth24 trunks=8,10,11,12,13,14,15,55')
    sw7.cmd('ovs-vsctl set port sw7-eth23 trunks=8,10,11,12,13,14,15,55')
    sw8.cmd('ovs-vsctl set port sw8-eth23 trunks=8,10,11,12,13,14,15,55')
    sw9.cmd('ovs-vsctl set port sw9-eth24 trunks=8,10,11,12,13,14,15,55')
    sw9.cmd('ovs-vsctl set port sw9-eth23 trunks=8,10,11,12,13,14,15,55')
    sw10.cmd('ovs-vsctl set port sw10-eth24 trunks=8,10,11,12,13,14,15,55')

    info("*** Adding VLAN Tagging \n")
    
    sw1_port_num = 4
    for i in range(1, sw1_port_num + 1):
        sw1.cmd('ovs-vsctl set port %i sw1-eth%d tag=11')

    sw2_port_num = 7
    for i in range(1, sw2_port_num + 1):
        sw2.cmd('ovs-vsctl set port %i sw2-eth%d tag=12')

    sw3_port_num = 4
    for i in range(1, sw3_port_num + 1):
        sw3.cmd('ovs-vsctl set port %i sw3-eth%d tag=15')

    sw4_port_num = 17
    for i in range(1, sw4_port_num + 1):
        sw4.cmd('ovs-vsctl set port %i sw4-eth%d tag=12')

    sw5_port_num = 22
    for i in range(1, sw5_port_num + 1):
        sw5.cmd('ovs-vsctl set port %i sw5-eth%d tag=12')

    sw5.cmd('ovs-vsctl set port sw5-eth23 tag=11')

    sw6_port_num = 22
    for i in range(1, sw6_port_num + 1):
        sw6.cmd('ovs-vsctl set port %i sw6-eth%d tag=12')

    sw6.cmd('ovs-vsctl set port sw6-eth23 tag=11')

    sw7_port_num = 12
    for i in range(1, sw7_port_num + 1):
        sw7.cmd('ovs-vsctl set port %i sw7-eth%d tag=12')

    sw8_port_num = 8
    for i in range(1, sw8_port_num + 1):
        sw8.cmd('ovs-vsctl set port %i sw8-eth%d tag=12')

    sw8.cmd('ovs-vsctl set port sw8-eth9 tag=11')

    sw9_port_num = 5
    for i in range(1, sw9_port_num + 1):
        sw9.cmd('ovs-vsctl set port %i sw9-eth%d tag=13')

    sw10_port_num = 21
    for i in range(1, sw10_port_num + 1):
        sw10.cmd('ovs-vsctl set port %i sw10-eth%d tag=13')
	...
	if controller_ip == ":
		controller_ip == ":
	net = Mininet(topo=topo,controller=lambda a:Re)
